...
function Level(specData, flowIdx, emissionIdx, emissionFreqStart, emissionFreqStop)
    ...
    specData(flowIdx).UserData.Emissions.Measures(emissionIdx).Level = struct( ...
        'FreqCenter_Min',  round(specData(flowIdx).Data{3}(frequencyIdx, 1), 1), ...
        'FreqCenter_Mean', round(specData(flowIdx).Data{3}(frequencyIdx, 2), 1), ...
        'FreqCenter_Max',  round(specData(flowIdx).Data{3}(frequencyIdx, 3), 1), ...
        'Channel_Min',     round(min(emissionPower),  1), ...
        'Channel_Mean',    round(mean(emissionPower), 1), ...
        'Channel_Max',     round(max(emissionPower),  1) ...
    );
end
...
function Occupancy(specData, flowIdx, emissionIdx, idx1, idx2)
    ...
    FCOChannel = 100 * sum(any(refInfMatrix, 1)) / width(refInfMatrix);
    ...
    integrationPeriod = specData(flowIdx).UserData.OccupancyFiniteIntegrationCache(cacheIndex).Parameters.IntegrationTime;

    % SAÍDA
    specData(flowIdx).UserData.Emissions.Measures(emissionIdx).FBO = struct( ...
        'Min', round(min(FBOPerSweep), 1), ...
        'Mean', round(mean(FBOPerSweep), 1), ...
        'Max', round(max(FBOPerSweep), 1) ...
    );
    ...
    specData(flowIdx).UserData.Emissions.Measures(emissionIdx).FCO = struct( ...
        ...
        'FreqCenter_Finite_Min', round(refFinMatrix(idxMatrixCenter, 1), 1), ...
        'FreqCenter_Finite_Mean', round(refFinMatrix(idxMatrixCenter, 2), 1), ...
        'FreqCenter_Finite_Max', round(refFinMatrix(idxMatrixCenter, 3), 1) ...
    );
end
