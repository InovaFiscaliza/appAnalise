classdef (Abstract) Table
    ...
    methods (Static)
        ...
        function tbl = Channel(reportInfo, analyzedData)
            ...
            for ii = 1:height(chTable)
                ...
                for jj = 1:emissionsCount
                    ...
                    emissionTable(jj, :) = { ...
                        sprintf('%s.%d.%d', reportInfo.Function.var_Index, ii, jj), ...
                        emissionFrequency, ...
                ...
                tbl(ii, :) = { ...
                    sprintf('%s.%d', reportInfo.Function.var_Index, ii), ...
                    chName, ...
            ...
            function tblTemplate = getTableTemplate(tblType, tblHeight)
                switch tblType
                    case 'GeneralTable'
                        tblTemplate = table( ...
                            'Size', [tblHeight, 17], ...
                             'VariableNames', {'ID', 'Transponder', 'Frequência central (MHz)', 'Largura (kHz)', 'Referência', 'FBO estimada (%)', 'Threshold mínimo', 'Threshold máximo', 'Offset', 'FBO mínima (%)', 'FBO média (%)', 'FBO máxima (%)', 'FCO (%)', 'FCO per bin (%)', 'Qtd. emissões', 'EmissionIdx', 'Emissões'}, ...
                             'VariableTypes', {'cell', 'cell', 'double', 'double', 'cell', 'double', 'double', 'double', 'double', 'double', 'double', 'double', 'double', 'cell', 'uint32', 'cell', 'cell'} ...
                         );
                    ...
                    case 'EmissionTable'
                        tblTemplate = table( ...
                            ...
                            'VariableTypes', {'cell', 'double', 'double', 'double', 'cell'} ...
                        );
                end
            end
        ...
        function jsonFileContent = scarabJsonFile(projectData, context, specData, correlationKey, executionMode, issueDetails)
            ...
            tasks = table( ...
                'Size', [numel(specData), 14], ...
                'VariableNames', {'correlationKey', 'bandId', 'sensor', 'latitude', 'longitude', 'freqStart', 'freqStop', 'measurementStartTime', 'measurementEndTime', 'measurementCount', 'levelUnit', 'taskType', 'taskDescription', 'relatedFiles'}, ...
                'VariableTypes', {'cell', 'double', 'cell', 'double', 'double', 'double', 'double', 'cell', 'cell', 'double', 'cell', 'cell', 'cell', 'cell'} ...
            );
            ...
            for ii = 1:numel(specData)
                tasks(ii, :) = { ...
                    ...
                    specData(ii).MetaData.LevelUnit, ...
